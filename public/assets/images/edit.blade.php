...
@section('content')
    <div class="content-wrapper p-3">
        ...
        <section class="content">
            <div class="container-fluid">
                <form class="form-horizontal" action="{{ route('myrequest.update', $request->id) }}" method="post">
                    {{ csrf_field() }}
                    @method('put')
                    <div class="row">
                        <div class="col-md-12">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9 mt-2">
                                            ...
                                            <div class="icheck-info d-inline mr-3">
                                                <input class="pr-type" type="radio" id="active{{ $item->id }}" name="category_id" value="{{ $item->id }}" {{ $item->id == $request->category_id ? 'checked':'' }}/>
                                        ...
                                        <div class="col-sm-3 input-group">
                                            <select class="form-control select2" name="category_sub_id" id="category_sub_id" required>
                                                ...
                                                @foreach ($category_sub as $item)
                                                    <option value="{{ $item->id }}" {{ $item->id == $request->category_sub_id ? 'selected':'' }}>{{ $item->category_name }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="input-group col-sm-3" id="item_availability_date" data-target-input="nearest">
                                            <input type="text" class="form-control datetimepicker-input" autocomplete="off" data-target="#item_availability_date" name="item_availability_date" value="{{ $request->item_availability_date }}" required/>
                                    ...
                                    <div class="form-group row d-none">
                                        ...
                                        <div class="col-sm-3 input-group">
                                            <select class="form-control select2" name="workplan_id" id="workplan_id" >
                                                <option value="">-- Silakan Pilih --</option>
                                                <option value="1" {{ $request->workplan_id == 1 ? 'selected':'' }}>item rencana kerja</option>
                                            </select>
                                            <!-- <div class="input-group-append">
                                                <div class="input-group-text"><i class="fas fa-user"></i></div>
                                            </div> -->
                                    ...
                                    <div class="form-group row d-none">
                                        ...
                                        <div class="col-sm-5 input-group">
                                            <select class="form-control select2" name="workplan_sub_id" id="workplan_sub_id" >
                                                <option value="">-- Silakan Pilih --</option>
                                                <option value="2" {{ $request->workplan_sub_id == 2 ? 'selected':'' }}>item sub rencana kerja</option>
                                            </select>
                                            <!-- <div class="input-group-append">
                                                <div class="input-group-text"><i class="fas fa-user"></i></div>
                                            </div> -->
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-6 input-group">
                                            <select class="form-control select2" name="item_budget_id" id="item_budget_id" required>
                                                <option value="">-- Silakan Pilih --</option>
                                                <option value="3" {{ $request->item_budget_id == 3 ? 'selected':'' }}>item budget</option>
                                            </select>
                                            <!-- <div class="input-group-append">
                                                <div class="input-group-text"><i class="fas fa-user"></i></div>
                                            </div> -->
                                                        ...
                                                        <tbody id="tbody">
                                                            @foreach ($request->details as $key => $detail)
                                                                <tr style="background-color: #dbdddb">
                                                                    <td>
                                                                        <input type='checkbox' name='record' data-id="{{ $detail->id }}">
                                                                        <input type='hidden' name='detail_id[]' value="{{ $detail->id }}">
                                                                    </td>
                                                                    <td><input class="form-control" name="item_name[]" size="50" value="{{ $detail->item_name}}" required /></td>
                                                                    <td>
                                                                        <textarea rows="3" cols="50" class="form-control" name="description[]">{{ $detail->description }}</textarea>
                                                                    </td>
                                                                    <td><input class="form-control" size="5" name="qty[]" value="{{ $detail->qty }}" required/></td>
                                                                    <td>
                                                                        <select class="form-control2 select2 tahun" name="unit[]">
                                                                            <option value="">-- Pilih --</option>
                                                                            @foreach($unit as $item)
                                                                            <option value="{{ $item->name }}" {{ $detail->unit == $item->name ? 'selected':'' }}>{{ $item->name }}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </td>
                                                                </tr>
        ...
        $(function() {
            ...
            $('.btn-add').click(function () {
                var markup = `<tr style="background-color: #dbdddb">
                                <td>
                                    <input type='checkbox' name='record' data-id="0">
            ...
            $('.btn-del').click(function() {
                $("table tbody").find('input[name="record"]').each(function(){
                    if($(this).is(":checked")){
                        var id = $(this).data('id')
                        if(id != 0) {
                            $.post('{{ route("myrequest.deletedetail") }}', {'_token':'{{ csrf_token() }}', 'detail_id':id}, function(result) {
                                console.log(result);
                            })
                        }
